/* aes_sbox.c - AES S-Box 및 역 S-Box 구현 */
#include <stdint.h>
...
#define SBOX_SIZE 256  /* S-Box 크기 (2^8) */
...
/**
 * @brief AES 역 S-Box (Inverse Substitution Box)
 * 
 * S-Box의 역변환으로, 복호화 시 사용됩니다.
 * S-Box와 역 S-Box는 서로 역함수 관계입니다.
 */
const uint8_t aes_inv_sbox[SBOX_SIZE] = {
    /* 0x00-0x0F */
 ...
 * 
 * 8비트 입력값을 S-Box를 통해 8비트 출력값으로 변환합니다.
 * 이는 AES의 비선형성을 제공하는 핵심 연산입니다.
 * 
 ...
 * 
 * 8비트 입력값을 역 S-Box를 통해 8비트 출력값으로 변환합니다.
 * 이는 S-Box의 역변환으로 복호화에 사용됩니다.
 * 
 * @param byte 입력 바이트 (0-255)
...
/*
 * 주의: SubBytes/InvSubBytes (상태 배열 전체 변환) 함수는
 * 에러 스택을 지원하는 구현이 `aes_round.c`에 존재합니다.
 * 여기서는 바이트 단위 치환 함수만 제공합니다.
 */